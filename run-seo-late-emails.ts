...
async function enrichLateEmails() {
  console.log('📬 Starting LATE PAYMENT EMAIL Enrichment (Live Product Edition)...');

  while (true) {
    // 1. Fetch rows: 🔥 THE FIX - Specifically target slugs starting with 'late-payment-email-'
    const { data: rows, error } = await supabase
      .from('seo_pages')
      .select('id, job_title, slug')
      .like('slug', 'late-payment-email-%') // Safe targeting
  ...
  while (attempts < 5) {
    try {
      ...
      const prompt = `
        ...
        3. deliverables: A JSON Array of EXACTLY 3 unique email body templates (strings) geared towards a ${row.job_title}:
        ...
        5. seo_desc: "Download professional, escalated late payment email templates for ${row.job_title}s. Stop chasing checks manually and automate your debt collection today."
      ...
      // Validate deliverables array length
