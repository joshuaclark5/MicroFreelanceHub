...
async function seedBatch18() {
  console.log('🌱 Planting seeds for Batch 18 (Contractor Agreements & Sign-Offs)...');
  ...
  professions.forEach((job) => {
    ...
    rowsToInsert.push({
      slug: `${baseSlug}-independent-contractor-agreement`,
      job_title: job,
      keyword: `${job.toLowerCase()} independent contractor agreement`,
      document_type: 'Independent Contractor Agreement',
      intent: 'transactional',
      batch_label: 'Batch 18'
    });

    rowsToInsert.push({
      slug: `${baseSlug}-project-sign-off-form`,
      job_title: job,
      keyword: `${job.toLowerCase()} project sign off form template`,
      document_type: 'Project Sign-Off Form',
      intent: 'transactional',
      batch_label: 'Batch 18'
    });
  });
