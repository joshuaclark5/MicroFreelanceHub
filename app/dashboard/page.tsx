...
import { 
  ...
  LayoutGrid, Repeat, Clock, DollarSign, TrendingUp, CheckCircle, PieChart,
  PenTool
} from 'lucide-react';
        ...
        <div className={sows.length > 0 ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "block"}>
          {sows.length === 0 ? (
            ...
            sows.map((sow) => {
               ...
               // 🧠 SMART STATUS LOGIC
               let statusLabel = sow.status;
               let statusColor = "bg-gray-50 text-gray-500 border-gray-100";
               let StatusIcon = null;

               if (isPaid) {
                   statusLabel = "Paid Full";
                   if (sched.type === 'split') statusLabel = "Installment Paid";
                   else if (sched.depositAmount) statusLabel = "Deposit Paid";
               ...
               >
                 ...
                 <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                    ...
                    <div className="flex flex-col items-end gap-2">
                       ...
                       <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wide border flex items-center gap-1 ${statusColor}`}>
                           {StatusIcon && <StatusIcon className="w-3 h-3" />} {statusLabel}
                       </span>
