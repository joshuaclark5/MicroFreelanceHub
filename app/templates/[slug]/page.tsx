...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const isInvoice = !isEmail && doc.document_type === 'Invoice';
  const label = isEmail ? 'Late Payment Emails' : (isInvoice ? 'Invoice' : 'Contract');
...
export default async function TemplatePage({ params }: { params: { slug: string } }) {
  ...
  // 🧠 THE CHAMELEON ENGINE LOGIC
  const isEmail = params.slug.startsWith('late-payment-email');
  const isInvoice = !isEmail && doc.document_type === 'Invoice';
  const isContract = !isEmail && !isInvoice;

  // Set Theme Colors
  const themeColor = isEmail ? 'indigo' : (isInvoice ? 'emerald' : 'blue');
  const badgeText = isEmail ? 'Email Templates' : (isInvoice ? 'Invoice Template' : 'Contract Template');
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 pb-20">
      ...
      <div className="bg-slate-900 text-white py-16 md:py-24 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-3 mb-6">
            <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${isEmail ? 'bg-indigo-600' : (isInvoice ? 'bg-emerald-600' : 'bg-blue-600')} text-white`}>
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 tracking-tight leading-tight">
            Free <span className={isEmail ? "text-indigo-400" : (isInvoice ? "text-emerald-400" : "text-blue-400")}>{title}</span> <br className="hidden md:block"/>
            <span className="text-white">{isEmail ? 'Late Payment Emails' : (isInvoice ? 'Invoice Template' : 'Service Agreement')}</span>
          </h1>
          ...
          {legalTip && (
             <div className={`max-w-2xl mx-auto border p-4 rounded-xl mb-8 flex gap-4 text-left ${isEmail ? 'bg-indigo-900/30 border-indigo-500/30' : (isInvoice ? 'bg-emerald-900/30 border-emerald-500/30' : 'bg-amber-500/10 border-amber-500/30')}`}>
                <div className={`p-2 rounded-lg shrink-0 h-fit ${isEmail ? 'bg-indigo-500/20' : (isInvoice ? 'bg-emerald-500/20' : 'bg-amber-500/20')}`}>
                   <Shield className={`w-5 h-5 ${isEmail ? 'text-indigo-400' : (isInvoice ? 'text-emerald-400' : 'text-amber-400')}`} />
                </div>
                <div>
                   <h3 className={`font-bold text-xs uppercase mb-1 ${isEmail ? 'text-indigo-400' : (isInvoice ? 'text-emerald-400' : 'text-amber-400')}`}>{isEmail ? 'Collections Tip' : (isInvoice ? 'Cash Flow Tip' : 'Pro Contractor Tip')}</h3>
                   <p className={`text-sm leading-relaxed ${isEmail ? 'text-indigo-100' : (isInvoice ? 'text-emerald-100' : 'text-amber-100')}`}>{legalTip}</p>
                </div>
             </div>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="flex flex-col">
          ...
          <div className="mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-4">
              {isEmail ? 'Why use an automated sequence?' : (isInvoice ? 'Why use a digital invoice?' : 'Why use a written agreement?')}
            </h2>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 md:p-8 mb-10">
            <h3 className="text-lg md:text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <span className={isEmail ? "text-indigo-600" : (isInvoice ? "text-emerald-600" : "text-blue-600")}>{isEmail ? '📬' : (isInvoice ? '💸' : '🛡️')}</span> What this sequence covers:
            </h3>
               ...
               {(isEmail 
                 ...
                 : ['Deliverables List', 'Payment Terms', 'IP Rights', 'Revision Limits', 'Cancellation Policy']
                  ...
                  <li key={i} className="flex gap-4 items-start">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 font-bold text-sm ${isEmail ? 'bg-indigo-100 text-indigo-600' : (isInvoice ? 'bg-emerald-100 text-emerald-600' : 'bg-green-100 text-green-600')}`}>✓</div>
          ...
          {/* Section 3: CTA Box */}
          <div className={`p-6 rounded-xl border mt-auto ${isEmail ? 'bg-indigo-50 border-indigo-100' : (isInvoice ? 'bg-emerald-50 border-emerald-100' : 'bg-blue-50 border-blue-100')}`}>
            <h4 className={`font-bold mb-2 text-lg ${isEmail ? 'text-indigo-900' : (isInvoice ? 'text-emerald-900' : 'text-blue-900')}`}>
        ...
        <div id="email-preview" className="relative lg:sticky lg:top-24 h-fit">
          <div className={`absolute inset-0 transform rotate-1 rounded-2xl ${isEmail ? 'bg-indigo-600/5' : (isInvoice ? 'bg-emerald-600/5' : 'bg-blue-600/5')}`}></div>
